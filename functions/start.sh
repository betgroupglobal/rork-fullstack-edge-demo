...
cleanup() {
  ...
  for name in gateway proxy-adapter; do
...
# --- Gateway -----------------------------------------------------------------
# Run the standalone Node.js HTTP server (server.js) instead of wrangler dev.
# server.js implements the full API surface without needing the Cloudflare
# Workers runtime, which is unavailable in this container.
