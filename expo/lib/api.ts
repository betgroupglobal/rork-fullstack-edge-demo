...
/**
 * Sends a captured login form to the worker and returns the compact YAML
 * phishlet in the PhishletGen-Automator format.
 */
export async function generateLoginPhishlet(
  ...
  const headers: Record<string, string> = { "Content-Type": "application/json" };
  if (authHeader) headers["Authorization"] = authHeader;
  const response = await safeFetch(`${BASE_URL}/api/proxies/${proxyId}/login-phishlet`, {
    method: "POST",
    headers,
    body: JSON.stringify(input),
  });
